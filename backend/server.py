...
def create_access_token(user_id: str, email: str) -> str:
    # 60 min access token — refresh interceptor on the frontend silently rotates
    # it when calls return 401, so the real session length is 7 days (refresh token).
...
async def refresh_token(request: Request, response: Response):
    ...
    try:
        ...
        # 60 min max_age matches the JWT expiry above
        response.set_cookie(key="access_token", value=access_token, httponly=True, secure=True, samesite="none", max_age=3600, path="/")
...
async def platform_wipe_org_data(
    ...
    if not org_id and not org_name:
        raise HTTPException(status_code=400, detail="Provide either org_id or org_name")
    ...
    if org_id:
        ...
        if not org:
            raise HTTPException(status_code=404, detail=f"No organization with id {org_id}")
        target_token = org_id.upper().replace(" ", "_")
    else:
        ...
        if len(matches) > 1:
            ...
            raise HTTPException(
                status_code=400,
                detail=f"Multiple organizations match '{norm}': {ids}. Use org_id to disambiguate.",
            )
        org = matches[0]
        target_token = norm.upper().replace(" ", "_")

    expected_token = f"YES_DELETE_{target_token}"
